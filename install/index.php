...
if($postback && $all_fields_provided && !count($general_errors))
{
    ...
    try
    {
        ...
        if($file)
        {
            ...
            fwrite($file, '// Configuration file generated by installer ' . date(DATE_RFC1123) . "\n\n");
            
            // Write each configuration value to the config.php file
            foreach($fields as $field)
                fwrite($file, '$config[\'' . $field->name . '\'] = "' . addslashes($_POST[$field->name]) . '";' . "\n");
            
            fclose($file);
            
            // TODO: execute the SQL statements that create the tables
            
            // Display the message
            echo "<p>Configuration file written successfully!</p><p>Please delete the 'install/' directory to continue.</p>";
            exit;
        }
        else
